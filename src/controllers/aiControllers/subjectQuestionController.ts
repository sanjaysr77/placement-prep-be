...
// 🔧 Normalize subject names (for consistent search)
...
async function generateSubjectQuestions(subject: string) {
  const prompt = `
Generate 3 interview questions for the subject "${subject}":
...
export async function getQuestionsForSubject(validatedSubject: string) {
  try {
    ...
    // 1️⃣ Embed the subject for semantic search
    const searchQuery = `Interview questions for ${canonicalSubject}`;
    ...
    if (matchedDocs.length > 0) {
      const matchedSubject = matchedDocs[0][0].metadata?.subject || canonicalSubject;
      ...
      const questions = shuffled
        .map(([doc]) =>
          doc.pageContent
            ...
            .replace(/\n*\s*Subject:.*$/i, "")  // remove trailing “Subject: …”
            .trim()
        )
        .slice(0, 3);

      console.log(
        `✅ Found ${matchedDocs.length} relevant matches for "${matchedSubject}". Showing 3 random ones.`
      );
      return { source: "pinecone", matchedSubject, questions };
    }

    // 3️⃣ If not found → generate new questions and store them
    console.log(`🆕 No semantic match found. Generating questions for "${canonicalSubject}"...`);
    ...
    const docsToStore = newQuestions.map((q) => ({
      ...
      metadata: { subject: canonicalSubject, type: "question" },
    }));

    await pineconeStore.addDocuments(docsToStore);
    console.log(`✅ Generated and stored ${newQuestions.length} new questions for "${canonicalSubject}"`);
